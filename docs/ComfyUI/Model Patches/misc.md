...
Functionally the same as `ModelSamplingSD3`, but implementation details mean that some model need to use this one while others the `SD3` one.

!!!note "Details"
    The two only differ in how they make the model represent `timestep`s.  
    Specifically, due to how the theory evolved, there are 2 conventions: represent `timestep` continuously as decimals from 0 to 1, or discretely as integers from 0 to 1000. `ModelSamplingAuraFlow` uses the former while `ModelSamplingSD3` uses the latter.
