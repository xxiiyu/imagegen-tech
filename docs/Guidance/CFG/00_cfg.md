...
For $0\leq\omega\leq1$, if the manifold is sufficiently curved between $x_\text{cond}$ and $x_\text{uncond}$ (exaggeratedly shown in the demo), it also leads to an off-manifold result. In practice however, it's fair to assume that the manifold isn't this curved (in other words, piecewise linear) and that $0\leq\omega\leq1$ should still be a 'good enough' distribution.

In any case, as CFG is mostly used with $\omega>1,$ it is therefore not well-grounded in theory; its use is mainly justified by the massive leap in image quality it offers in practice.
