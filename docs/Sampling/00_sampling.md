...
- **[#What](../Models/00_models.md#what-does-generating-mean):** sampling means to pick data out according to a data distribution $p_\text{data},$ which assigns probabilities to data. For example, a cat distribution $p_\text{Cat}$ would assign high probability to cats, and low probability to everything else like dogs, humans, scenery, ...
...
1. `k-diffusion` works with a denoiser model $D_\theta$ that would predict the clean image directly, rather than $u_\theta$ which predicts the change needed to get there. 
...
!!!info "Algorithm 2: `k-diffusion` Sampling (Euler-Maruyama Method)"
    **Require:** data prediction model $D_\theta,$ noise schedule $[\sigma_0,\sigma_1,...,\sigma_n]$, diffusion coefficient $\eta$
    ...
    5. &emsp;Set $u_\theta\gets (x-D_\theta(x,\sigma_i))/\sigma_i$
    6. &emsp;Update $x\gets x+h_\text{down}u_\theta+h_\text{up}\epsilon$
    7. **end for** 
    8. **return** $x$

!!!info "Algorithm 2.1: `k-diffusion` Sampling Truer-to-the-Code (Euler-Maruyama Method)"

    **Require:** denoiser model $D_\theta,$ noise schedule $[\sigma_0,\sigma_1,...,\sigma_n]$, diffusion coefficient $\eta$

    1. Draw a sample $x\sim\mathcal N(0, \sigma_0^2)$
    2. **for** $i=0,...,n-1$ **do**
    3. &emsp;Draw random noise $\epsilon\sim\mathcal N(0,1)$
    4. &emsp;Set $\sigma_\text{down},\sigma_\text{up}\gets\text{get\_ancestral\_step}(\sigma_i,\sigma_{i+1},\eta)$ 
    6. &emsp;Set $d\gets (x-D_\theta(x,\sigma_i))/\sigma_i$
